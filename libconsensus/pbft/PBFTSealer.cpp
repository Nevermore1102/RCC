...
void PBFTSealer::handleBlock()
{
    ...
    /// in case of the block is generated by the nextleader and the transaction num is over maxTransactionLimit
    ...
    //只有生成的合法的block之后,编号才能往上加,一开始节点都生成的是空块,所以blk_num一直是1
...
//                             << LOG_KV("hash", m_sealing.block->header().hash().abridged());
//    }else{
//        PBFTSEALER_LOG(INFO) << LOG_DESC("m_pbftEngine->shouldSeal()为假")
//                             << LOG_KV("blkNum", m_sealing.block->header().number())
//                             << LOG_KV("nodeIdx", m_pbftEngine->nodeIdx())
//                             << LOG_KV("hash", m_sealing.block->header().hash().abridged());
//    }
